...
router.post("/jobs", jwtAuth, (req, res) => {
  ...
  if (user.type != "recruiter") {
    res.status(401).json({
      message: "You don't have permissions to add classes",
    });
    return;
  }
  ...
  job
    .save()
    .then(() => {
      res.json({ message: "Class added successfully to the database" });
    })
    .catch((err) => {
...
router.put("/jobs/:id", jwtAuth, (req, res) => {
  ...
  if (user.type != "recruiter") {
    res.status(401).json({
      message: "You don't have permissions to change the class details",
    });
    return;
  }
  Job.findOne({
...
router.delete("/jobs/:id", jwtAuth, (req, res) => {
  ...
  if (user.type != "recruiter") {
    res.status(401).json({
      message: "You don't have permissions to delete the class",
    });
    return;
  }
...
router.post("/jobs/:id/applications", jwtAuth, (req, res) => {
  ...
  if (user.type != "applicant") {
    res.status(401).json({
      message: "You don't have permissions to apply for a class",
    });
    return;
  }
  const data = req.body;
          ...
          Application.countDocuments({
            ...
            .then((activeApplicationCount) => {
              if (activeApplicationCount < job.maxApplicants) {
                      ...
                      Application.countDocuments({
                        ...
                        if (acceptedJobs === 0) {
                          ...
                          application
                            ...
                            .then(() => {
                              res.json({
                                message: "Class appointment successful",
                              });
                            })
                            .catch((err) => {
                              res.status(400).json(err);
                          ...
                          res.status(400).json({
                            message:
                              "You already have an appointment. Hence you cannot apply.",
                          });
                        }
                      });
                    } else {
                ...
                res.status(400).json({
                  message: "Appointment limit reached",
                });
              }
            })
            .catch((err) => {
...
router.get("/jobs/:id/applications", jwtAuth, (req, res) => {
  ...
  if (user.type != "recruiter") {
    res.status(401).json({
      message: "You don't have permissions to view class appointments",
    });
    return;
  }
  const jobId = req.params.id;
...
router.put("/applications/:id", jwtAuth, (req, res) => {
  ...
  if (user.type === "recruiter") {
    if (status === "accepted") {
      ...
      Application.findOneAndUpdate(
        ...
        .then((application) => {
          if (application === null) {
            res.status(400).json({
              message: "Appointment status cannot be updated",
            });
            return;
          }
          if (status === "finished") {
    ...
    if (status === "cancelled") {
      ...
      res.status(401).json({
        message: "You don't have permissions to update class status",
      });
    }
  }
});
...
router.get("/applicants", jwtAuth, (req, res) => {
  ...
  if (user.type === "recruiter") {
    ...
    res.status(400).json({
      message: "You are not allowed to access students list",
    });
  }
});
